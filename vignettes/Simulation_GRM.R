...
erg <- biclust(as.matrix(snp.pine), method=BCCC(), delta=1.5, alpha=1, number=)
erg

fit <- flashier::flash(snp.pine,   greedy_Kmax = 20L, backfit = T)

plot(fit, pw_which = "factors")

fit$flash_fit$Y
low_fit <- fit$L_pm %*% t(fit$F_pm)
low_fit <- flashier::ldf(fit)
low_fit

y <- low_fit$L %*% diag(low_fit$D) %*% t(low_fit$F)


fit <- flashier::flash(Matrix(GRM),   greedy_Kmax = 300L, backfit = F)
...
mt_ebmf <- function(n, grm, n_resp, resp.m = NULL, model, formula = NULL, data = NULL){
  ...
  fit <- flashier::flash(Matrix(grm),   greedy_Kmax = 110,
                         backfit = F)

  ldf <- flashier::ldf(fit)
...
mat <- mt_ebmf(grm = GRM, n_resp = 3, resp.m = as.data.frame(pheno_col),  model = "AE", data = data)
